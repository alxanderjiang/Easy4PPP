...
def SPP_from_IGS(obs_mat,obs_index,IGS,CLK,sat_out,ion_param,sat_pcos,sol_mode='SF',f1=1575.42*1e6,f2=1227.60*1e6,el_threthod=7.0,obslist=[],pre_rr=[]):
    ...
    while(1):
        ...
        #print(dresult)
        ls_count+=1
        if(abs(dresult[0])<1e-4 and abs(dresult[1])<1e-4 and abs(dresult[2])<1e-4 or ls_count>100):
            #估计先验精度因子
            break
...
if __name__=='__main__':
    ...
    dcb_file_0="data/DCB/CAS0MGXRAP_20241310000_01D_01D_DCB.BSX" #频间偏差文件, 支持预先转换后的.npy格式
    dcb_file_1=""
    dcb_file_2=""
    ...
    f1=1575.42E+6                                  #配置第一频率
    f2=1227.60E+6                                  #配置第二频率

    el_threthod=15.0                                 #设置截止高度角
    ex_threshold_v=30                               #设置先验残差阈值
    ex_threshold_v_sigma=10                          #设置后验残差阈值
    Mw_threshold=2.5                                #设置Mw组合周跳检验阈值
    GF_threshold=0.15                               #设置GF组合周跳检验阈值
