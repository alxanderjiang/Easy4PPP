...
#作者: 蒋卓君, 武汉理工大学&复旦大学
#Copyright 2025-, by Zhuojun Jiang, Wuhan University of Technology & Fudan University, China
...
def SPP_from_IGS(obs_mat,obs_index,IGS,CLK,sat_out,ion_param,sat_pcos,sol_mode='SF',f1=1575.42*1e6,f2=1227.60*1e6,el_threthod=7.0,obslist=[],pre_rr=[]):
    ...
    for i in range(0,sat_num):
        ...
        rs1=insert_satpos_froom_sp3(IGS,rt_unix-p1/clight,si_PRN)    #观测历元卫星位置
        rs2=insert_satpos_froom_sp3(IGS,rt_unix-p1/clight+dt,si_PRN) #插值求解卫星速度矢量
        rs=[rs1[si_PRN][0],rs1[si_PRN][1],rs1[si_PRN][2]]
        dts=insert_clk_from_sp3(CLK,rt_unix-p1/clight,si_PRN)[si_PRN]
        ...
        rs1=insert_satpos_froom_sp3(IGS,rt_unix-p1/clight-dts,si_PRN)    #观测历元卫星位置
        rs2=insert_satpos_froom_sp3(IGS,rt_unix-p1/clight-dts+dt,si_PRN) #插值求解卫星速度矢量
        rs=[rs1[si_PRN][0],rs1[si_PRN][1],rs1[si_PRN][2]]
        dts=insert_clk_from_sp3(CLK,rt_unix-p1/clight-dts,si_PRN)[si_PRN]
        drts=insert_clk_from_sp3(CLK,rt_unix-p1/clight-dts,si_PRN)[si_PRN]
        drs=[(rs2[si_PRN][0]-rs[0])/dt,(rs2[si_PRN][1]-rs[1])/dt,(rs2[si_PRN][2]-rs[2])/dt]
        dF=-2/clight/clight*( rs[0]*drs[0]+rs[1]*drs[1]+rs[2]*drs[2] )      #利用精密星历进行相对论效应改正
        dts=dts+dF
    ...
    while(1):
        ...
        for i in range(0,sat_num):
            ...
            '''#不同电离层模型
            if(ion_mode=='Klobuchar'):
                Z[i]=p1-r0-satpos.get_Tropdelay(rr,rs)-satpos.get_ion_GPS(rt_unix,rr,rs,ion_param)+clight*dts+dF
            elif(ion_mode=='STEC'):
                Z[i]=p1-r0-satpos.get_Tropdelay(rr,rs)-stecdict[si_PRN]*40.28/f_GHZ/f_GHZ+clight*dts+dF
            elif(ion_mode=='Ion-free'):
                Z[i]=p_IF-r0-satpos.get_Tropdelay(rr,rs)+clight*dts+dF
            elif(ion_mode=='BDGIM'):
                Z[i]=p1-r0-satpos.get_Tropdelay(rr,rs)-satpos.get_BDSGIM(rt_unix,ion_param,rr,rs,MF_mode=1)*40.28/f_GHZ/f_GHZ+clight*dts+dF
            else:
                Z[i]=p1-r0-satpos.get_Tropdelay(rr,rs)+clight*dts+dF'''
...
def createKF_HRZ_new(obslist,rt_unix,X,X_time,Pk,Qk,ion_param,phase_bias,peph_sat_pos,f1=1575.42*1e6,f2=1227.60*1e6,ex_threshold_v=30,exthreshold_v_sigma=4,post=True):
    ...
    for i in range(sat_num): #逐卫星按行创建设计矩阵
        ...
        if(post==True):
            out_v=[]
            if abs(v[i*4])>ex_threshold_v_sigma*sqrt(R[i*4][i*4]): 
                #print(si_PRN," 验后方差校验不通过",v[i*4],4*sqrt(R[i*4][i*4]))
                out_v.append(v[i*4])
            if abs(v[i*4+1])>ex_threshold_v_sigma*sqrt(R[i*4+1][i*4+1]):
                #print(si_PRN," 验后方差校验不通过",v[i*4+1],4*sqrt(R[i*4+1][i*4+1]))
                out_v.append(v[i*4+1])
            if abs(v[i*4+2])>ex_threshold_v_sigma*sqrt(R[i*4+2][i*4+2]): 
                #print(si_PRN," 验后方差校验不通过",v[i*4],v[i*4+2],4*sqrt(R[i*4+2][i*4+2]))
                out_v.append(v[i*4+2])
            if abs(v[i*4+3])>ex_threshold_v_sigma*sqrt(R[i*4+3][i*4+3]):
...
def log2out(rt_unix,v,obslist,X,X_time,Pk,peph_sat_pos,f1=1575.42*1e6):
    ...
    for i in range(len(obslist)):
        ...
        out[si_PRN]['res_p1']=v[4*i]
        out[si_PRN]['res_l1']=v[4*i+1]
        out[si_PRN]['res_p2']=v[4*i+2]
        out[si_PRN]['res_l2']=v[4*i+3]
...
if __name__=='__main__':
    ...
    SP3_file=r"data\NAVdata\Peph\20250350\WUM0MGXFIN_20250350000_01D_05M_ORB.SP3"
    CLK_file=r"data\NAVdata\Peph\20250350\WUM0MGXFIN_20250350000_01D_30S_CLK.CLK"
    ATX_file=r"data\ATX\igs20.atx"                  #天线文件, 支持格式转换后的npy文件和IGS天线文件
    BRDC_file=r"C:\Users\71793\Desktop\UB482 User_series\SerialPort\QUXian\brdc0350_8.25n"     #广播星历文件, 支持BRDC/RINEX3混合星历
    ion_param=[]                                    #手动设置广播星历电离层参数      
                                                    #若电离层参数为空,则由广播星历文件给出
    ...
    dcb_file_1="C1C_C1W_DCB.npy"                    #第一频率码间偏差, 同上
    dcb_file_2="C2C_C2W_DCB.npy"                    #第二频率码间偏差, 同上
